...
@dataclass
class Puzzle:
    """Define um puzzle: categorias, valores e restrições."""
    n: int                                    # número de posições
    categories: dict[str, list[str]]          # {"Cor": ["Branca","Verde",...], ...}
    clues: list[dict] = field(default_factory=list)

    # Atalhos para os nomes das categorias e valores
...
class EinsteinModel:
    ...
    def at_position(self, cat, val, pos: int):
        """val está na posição fixa pos (1-indexed)."""
        self.model.add(self.pos[cat][val] == pos - 1)

    def at_end(self, cat, val):
        """val está numa das pontas (pos 0 ou n-1)."""
    ...
    def between_ordered(self, cat_mid, val_mid, cat_left, val_left, cat_right, val_right):
        """val_left < val_mid < val_right (nessa ordem)."""
    ...
    def between_unordered(self, cat_mid, val_mid, cat_a, val_a, cat_b, val_b):
        """val_mid está entre val_a e val_b (qualquer ordem)."""
        # (a < mid < b) OU (b < mid < a)
        b = self.model.new_bool_var("")
    ...
    def solve(self) -> dict | None:
        """Retorna a primeira solução ou None."""
    ...
    def count_solutions(self, limit=2) -> int:
        """Conta soluções até o limite (para verificar unicidade)."""
        solver = cp_model.CpSolver()
