...
O puzzle: N locais da investigação, N suspeitos, N armas, N motivos, N horários.
Dicas espalhadas pelos terminais permitem deduzir a solução única.

Uso:
  python3 noir_puzzle_gen.py --seed 42 --difficulty hard
  python3 noir_puzzle_gen.py --seed 7 --difficulty easy --output puzzle_data.json
  python3 noir_puzzle_gen.py --seed 1 --difficulty hard --merge src/game_data.json
"""
...
def merge_puzzle_into_game_data(game_data_path: str, puzzle_interactables: list[dict],
    ...
    def find_free_position(room_id: str, width: float, height: float, existing: list[dict]) -> tuple:
        for attempt_y in range(20, 90, 8):
            for attempt_x in range(5, 95 - int(width), 10):
                ...
                for e in existing:
                    ...
                    if x < ex2 and x2 > ex1 and y < ey2 and y2 > ey1:
                        overlap = True
                        break
                if not overlap:
                    return (x, y)
...
def main():
    ...
    parser.add_argument("--seed", "-s", type=int, default=42, help="Seed para reprodutibilidade")
    parser.add_argument("--difficulty", "-d", choices=["easy", "medium", "hard"], default="medium",
        help="Dificuldade: easy (18+ pistas), medium (14+), hard (10+)")
    parser.add_argument("--rooms", "-n", type=int, default=0, help="Número de posições (0=auto por dificuldade)")
