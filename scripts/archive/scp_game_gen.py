...
ARQUIVO MORTO — Este script pertence a um jogo SCP Wiki diferente
do jogo noir "Murphy Law". Mantido apenas para referência.

Uso:
python3 -m scripts.archive.scp_game_gen 173 049 096 682
python3 -m scripts.archive.scp_game_gen 173 049 096 --output meu_mapa.json
python3 -m scripts.archive.scp_game_gen 173 049 096 --mock
"""
...
CLASS_TO_LEVEL: dict[str, int] = {
    ...
    "thaumiel": 3, "apollyon": 3, "neutralized": 1,
}

CLASS_ICON: dict[str, str] = {
    "safe": "ShieldCheck", "euclid": "Ghost",
    "keter": "Skull", "thaumiel": "Eye", "apollyon": "Flame",
}
...
def parse_scp(soup: BeautifulSoup, number: str) -> dict:
    ...
    object_class = object_class_raw.split()[0].lower()
...
def build_rooms(scps: list[dict]) -> dict:
    ...
    rooms["entrance"] = {
        ...
        "interactables": [
            {"id": "door_corridor", "x": 50, "y": 30, "icon": "DoorClosed", "type": "travel",
             "label": "Porta para o Corredor Leste", "requiredItem": "keycard_1",
             "failedMessage": "[ACESSO NEGADO] Requer Cartão de Acesso Nível 1.",
             "successMessage": "O leitor apitou verde. Porta destrancada.", "targetRoom": "corridor"},
            {"id": "guard_desk", "x": 20, "y": 70, "icon": "Search", "type": "inspect",
             "label": "Mesa do Segurança",
             "description": "Um segurança morto repousa sobre o teclado. O pescoço dele está quebrado de um jeito não natural."},
            {"id": "keycard_1_pickup", "x": 25, "y": 75, "icon": "Key", "type": "pickup",
             "label": "Cartão Nível 1", "description": "Você pegou o Cartão de Acesso Nível 1 caído debaixo da mesa.",
             "pickupItem": "keycard_1", "hideAfterInteract": True},
        ],
    }

    corridor_doors: list[dict] = [
        {"id": "door_entrance", "x": 50, "y": 90, "icon": "DoorOpen", "type": "travel",
         "label": "Voltar para Triagem", "targetRoom": "entrance"}
    ]
    ...
    for i, scp in enumerate(scps):
        ...
        door: dict = {
            "id": f"door_{room_id}", "x": x_pos, "y": y_pos,
            "icon": "DoorClosed" if level > 1 else "DoorOpen",
            "type": "travel", "label": f"Sala de Contenção — SCP-{scp['number']}",
            "targetRoom": room_id,
        }
        if level > 1:
    ...
    corridor_doors.append({
        "id": "door_server", "x": 80, "y": 20, "icon": "Archive", "type": "travel",
        "label": "Sala dos Servidores", "requiredItem": "keycard_2",
        "failedMessage": "[ACESSO NEGADO] Requer Cartão de Acesso Nível 2.", "targetRoom": "server_room",
    })
    corridor_doors.append({
        "id": "door_containment", "x": 20, "y": 20, "icon": "Lock", "type": "travel",
        "label": "Ala de Contenção Euclidiana", "requiredItem": "keycard_3",
        "failedMessage": "[ISOLAMENTO] Requer Cartão de Acesso Nível 3.",
        "successMessage": "Trancas pneumáticas liberadas. Entrando no setor de risco...", "targetRoom": "containment",
    })

    rooms["corridor"] = {
        "id": "corridor",
        "name": "Corredor Leste — Nível 2",
        "description": "Lâmpadas fluorescentes estouradas. Marcas nas paredes como se alguém tivesse tentado se segurar com força. O silêncio pesa.",
    ...
    for scp in scps:
        ...
        interactables = [
            {"id": f"door_back_{room_id}", "x": 50, "y": 90, "icon": "DoorOpen", "type": "travel",
             "label": "Voltar para o Corredor", "targetRoom": "corridor"},
            {"id": f"cell_{room_id}", "x": 50, "y": 30, "icon": icon, "type": "inspect",
             "label": f"Cela de Contenção — SCP-{scp['number']}",
             "description": scp["description"] or "A cela está escura demais para ver qualquer coisa."},
            {"id": f"terminal_{room_id}", "x": 20, "y": 50, "icon": "FileText", "type": "terminal_read",
        ...
        if level >= 2:
            ...
            interactables.append({
                "id": f"keycard_{next_level}_pickup_{room_id}", "x": 75, "y": 65,
                "icon": "Key", "type": "pickup", "label": f"Cartão Nível {next_level}",
                "description": f"Você encontrou um Cartão de Acesso Nível {next_level} nesta sala.",
                "pickupItem": f"keycard_{next_level}", "hideAfterInteract": True,
            })

        rooms[room_id] = {
            "id": room_id,
            "name": f"Câmara de Contenção — SCP-{scp['number']} [{obj_class.upper()}]",
            "description": f"Esta sala abrigava o SCP-{scp['number']}. Marcas de contenção reforçada cobrem as paredes.",
    ...
    rooms["server_room"] = {
        ...
        "interactables": [
            {"id": "door_office_back", "x": 50, "y": 90, "icon": "DoorOpen", "type": "travel",
             "label": "Voltar para Corredor", "targetRoom": "corridor"},
            {"id": "server_terminal", "x": 50, "y": 30, "icon": "Database", "type": "terminal_read",
             "label": "SISTEMA DE CONTENÇÃO PRINCIPAL",
             "documentData": {
                 "title": "SISTEMA SCP OS",
                 "content": [
                     "> ACESSANDO DADOS...", "> ALERTA: SISTEMA COMPROMETIDO", "",
                     *[f"SCP-{s['number']}: CONTENÇÃO {'ATIVA' if random.random() > 0.5 else 'FALHOU — LOCALIZAÇÃO DESCONHECIDA'}" for s in scps],
                     "", ">> ACESSE A ALA DE CONTENÇÃO EUCLIDIANA COM URGÊNCIA. <<",
                 ],
             }},
            {"id": "keycard_3_pickup", "x": 80, "y": 70, "icon": "Key", "type": "pickup",
             "label": "Cartão Nível 3", "description": "Você encontrou o Cartão de Acesso Nível 3 deixado por um pesquisador.",
             "pickupItem": "keycard_3", "hideAfterInteract": True},
        ],
    }
    ...
    rooms["containment"] = {
        ...
        "description": "Sombras oscilam. Portas de contenção abertas. O silêncio aqui é diferente — é o tipo que precede algo.",
        "interactables": [
            {"id": "door_corridor_back", "x": 50, "y": 90, "icon": "DoorOpen", "type": "travel",
             "label": "Voltar para Corredor (Fuga)", "targetRoom": "corridor"},
            {"id": "containment_cell", "x": 50, "y": 30, "icon": "Ghost", "type": "inspect",
             "label": "Cela Central de Contenção",
             "description": "Vazia. Traços biológicos cobrem o chão. Você ouve algo se mover atrás de você. NÃO SE VIRE."},
            {"id": "finale_terminal", "x": 20, "y": 50, "icon": "Terminal", "type": "terminal_read",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Gera game_data.json a partir da SCP Foundation Wiki.")
    parser.add_argument("numbers", nargs="+", metavar="SCP_NUMBER",
                        help="Números dos SCPs para incluir (ex: 173 049 096)")
    parser.add_argument("--output", "-o", default="game_data.json", metavar="FILE",
                        help="Arquivo JSON de saída (padrão: game_data.json)")
    parser.add_argument("--delay", "-d", type=float, default=REQUEST_DELAY, metavar="SEGUNDOS",
    ...
    for number in args.numbers:
        ...
        if args.mock:
            ...
            scps.append({
                "number": normalized, "title": f"SCP-{normalized}", "object_class": obj_class,
    ...
    game_data = {"ITEM_NAMES": build_item_names(scps), "GAME_ROOMS": build_rooms(scps)}
